...
RUN RUSTFLAGS="$(cat /flags.txt)" cargo chef cook --target "$(cat /target.txt)" --release --no-default-features --features "sqlite foundationdb postgres mysql rocks s3 redis azure nats zenoh  kafka enterprise" --recipe-path /recipe.json
COPY . .
RUN RUSTFLAGS="$(cat /flags.txt)" cargo build --target "$(cat /target.txt)" --release -p stalwart --no-default-features --features "sqlite foundationdb postgres mysql rocks s3 redis azure nats zenoh  kafka enterprise"
